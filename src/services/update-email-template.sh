...
# This script updates the confirmation email template with the dark theme version
...
# Find the start and end of the generateConfirmationEmailHTML function
START_LINE=$(grep -n "^function generateConfirmationEmailHTML" "$FILE" | head -1 | cut -d: -f1)
# Find the closing of this function (the next function declaration or EOF)
END_LINE=$(tail -n +$((START_LINE + 1)) "$FILE" | grep -n "^function\|^// Generate plain text" | head -1 | cut -d: -f1)
END_LINE=$((START_LINE + END_LINE - 1))

echo "Updating lines $START_LINE to $END_LINE in $FILE"
echo "Creating backup..."

# We'll manually create the new function content
cat > /tmp/new_confirmation_function.txt << 'TEMPLATE_EOF'
...
echo "Replacement script created successfully!"
echo "Please manually update the function in emailService.ts with the content from /tmp/new_confirmation_function.txt"
